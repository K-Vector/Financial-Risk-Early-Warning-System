...
def generate_dummy_data(output_path):
    ...
    rolling_threshold = df['HY_SPREAD'].rolling(window=52, min_periods=1).quantile(0.75) # 252 trading days in a year
    
      # 1 = FSI above 75th percentile (stress event)
    # We use a slight lag to avoid look-ahead bias, but for simplicity in dummy data, we'll use the current value
